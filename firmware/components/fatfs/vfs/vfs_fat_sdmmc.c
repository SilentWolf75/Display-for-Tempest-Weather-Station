                                ...
                                const char *drv, sdmmc_card_t *card, BYTE pdrv)
{
    ...
    workbuf = heap_caps_malloc(workbuf_size, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (workbuf == NULL) {
        return ESP_ERR_NO_MEM;
    }
    ...
    const MKFS_PARM opt = {(BYTE)FM_FAT32, (mount_config->use_one_fat ? 1 : 2), 0, 0, 0};
...
esp_err_t esp_vfs_fat_sdcard_format_cfg(const char *base_path, sdmmc_card_t *card, esp_vfs_fat_mount_config_t *cfg)
{
    ...
    const size_t workbuf_size = 4096;
    void *workbuf = heap_caps_malloc(workbuf_size, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (workbuf == NULL) {
        return ESP_ERR_NO_MEM;
    }
    ...
    const MKFS_PARM opt = {(BYTE)FM_FAT32, (s_ctx[id]->mount_config.use_one_fat ? 1 : 2), 0, 0, 0};
    res = f_mkfs(drv, &opt, workbuf, workbuf_size);
    free(workbuf);
    if (res != FR_OK) {
        ret = ESP_FAIL;
