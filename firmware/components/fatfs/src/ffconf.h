...
/* LOCAL CHANGE (Tempest Weather Display): exFAT enabled.
 * Upstream ESP-IDF ships this as 0 and exposes no Kconfig for it, and the
 * #define is not #ifndef-guarded, so a -D on the command line cannot reach
 * it. Overriding it is the only reason this component is vendored at all.
 * exFAT also needs FF_USE_LFN >= 1, which comes from CONFIG_FATFS_LFN_HEAP
 * in sdkconfig.defaults -- without that this constant does nothing. */
