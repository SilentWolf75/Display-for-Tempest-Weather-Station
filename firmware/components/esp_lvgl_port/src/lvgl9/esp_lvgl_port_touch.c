...
static void lvgl_port_touchpad_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    ...
    /* LOCAL CHANGE (Tempest Weather Display): do not abort on a failed read.
     *
     * Upstream wraps both of these in ESP_ERROR_CHECK, so a single transient
     * I2C error on the touch controller calls abort() and takes the whole
     * panel down. On this board the GT911 shares its bus with the indoor
     * temperature sensor, and that does occasionally glitch:
     *
     *   GT911: esp_lcd_touch_gt911_read_data(232): I2C read error!
     *   ESP_ERROR_CHECK failed: ESP_ERR_INVALID_STATE ... abort()
     *
     * A touch controller that misses a poll should drop that frame of input,
