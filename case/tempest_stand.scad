...
// NO heat-set inserts, and no bosses to hold them.
//
// The corner screw holes sit at (3.10, 3.00). The LCD module is 235.5 x 143.5
// centred on a 247.04 x 147.01 board, so its edge is only 1.75 mm in from the
// board edge, putting the screw hole just 1.25 mm from the module. A boss of
// ANY diameter over 2.49 mm therefore overlaps the screen, and an M3 insert
// needs 7.4 mm. There is nowhere to put one -- this is geometry, not fit.
//
// So the screw threads straight into the bezel's own face. That is why the
// face is 7.0 mm rather than 4.5: the thread has to live there and nowhere
// else, and an M3 x 30 lands 5.6 mm into it.
SCREW_PILOT    = 2.5;   // self-tapping M3 in plastic
SCREW_PILOT_DP = 6.0;   // blind, leaving 1.0 mm of skin at the front
...
BEZEL_T    = 7.0;       // face thickness. Set by the screw, not by looks: an
                        // M3 x 30 lands 5.6 mm into it, plus 1.0 mm of skin
                        // so nothing shows through the front.
...
module bezel() {
    difference() {
        ...
        // The window: a straight cut. The 45-degree relief that used to
        // ring it printed badly, so it is gone.
        translate([ACTIVE_X + BEZEL_LIP, ACTIVE_Y + BEZEL_LIP, -1])
            cube([ACTIVE_W - 2*BEZEL_LIP, ACTIVE_H - 2*BEZEL_LIP, BEZEL_T + 2]);

        // Blind pilots for the screws, drilled from the BACK into the face.
        // They stop short of the front so nothing shows through.
        for (h = holes)
            translate([h[0], h[1], BEZEL_T - SCREW_PILOT_DP])
                cylinder(d=SCREW_PILOT, h=SCREW_PILOT_DP + 0.1, $fn=32);
    }
}
...
// The thread lives in the bezel FACE now, one glass height above the PCB.
// Reaching it takes a long screw; overshooting punches out through the front.
SCREW_MIN  = PCB_Z + PCB_ONLY + FRONT_GLASS - SCREW_HEAD_SEAT;
SCREW_MAX  = SCREW_MIN + SCREW_PILOT_DP;
echo(str("M3 BUTTON head (under-head length): at least ", SCREW_MIN,
         " to reach the bezel, at most ", SCREW_MAX,
         " before it breaks out of the front. M3 x 30 is the fit."));
echo(str("Bezel skin in front of each screw: ",
         BEZEL_T - SCREW_PILOT_DP, " mm"));
